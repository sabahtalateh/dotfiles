...
Фреймоврк для настройки `zsh`. После установке терминал сразу станет разноцветный. `omz` так-же перезапишет текущий `~/.zshrc` файл, если там есть что-то полезное, нужно его скопировать
...
:warning: После установки будет предложено добавить `$PATH`-переменные для того чтобы `brew` был доступен из терминала, этого делать не нужно, переменные будут добавлены после добавления файла `.dotfiles/zsh/zshrc` в `~/.zshrc` на одном из следующих шагов. Пока что `brew` будет доступен только по полному пути `/opt/homebrew/bin/brew` (для `arm` систем)
