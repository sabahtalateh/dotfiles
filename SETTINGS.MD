...
После настройки сочетаний клавиш [Move focus to next window](#keyboard-1) можно будет переключаться между окнами одного пространства `Stage Manager`-а при помощи `Cmd+[backtick]`. Так-же ниже включим режим `Reduce Motion` чтобы окна не летали как бешеные при переключении
