...
then
    echo "VSCode Insiders not installed"
    exit 1
fi

# Run from the script's own directory so relative paths resolve from any cwd
cd "$(dirname "$0")" || exit 1
...
echo "Install VSCode Insiders extensions"
xargs -L 1 code-insiders --force --install-extension < vscode/extensions.txt
